...
import { useAdminInterviewSession } from "@/lib/hooks/useAdmin";
...
export default function AdminInterviewDetailPage() {
  ...
  const { data: session, isLoading, isError } =
    useAdminInterviewSession(sessionId);

  if (isLoading) return <LoadingSpinner />;
  if (isError || !session) {
    return <ErrorMessage message="Failed to load session details" />;
  }
    ...
    <div className="mx-auto max-w-4xl space-y-6 px-6 py-8">
      ...
      <div>
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          ...
          {session.participantName?.trim()
            ...
            : ""}
          {session.participantEmail ?? session.userId ?? "Unknown"}
        </p>
      </div>
      ...
      <Card className="p-6">
        <div className="grid grid-cols-2 gap-6 md:grid-cols-4">
          ...
          <div>
            <p className="mb-1 text-sm text-muted-foreground">Questions</p>
            <p className="text-lg font-semibold text-foreground">
                  ...
                  <div>
                    ...
                    <div className="rounded bg-muted p-3">
                      ...
                      <div className="mt-2 flex items-center gap-2 text-xs text-muted-foreground">
                        ...
                        <span>
                          Duration:{" "}
                          {formatElapsedSeconds(answer.audioDuration ?? 0)}
                        </span>
                      </div>
                    </div>
                  </div>
      ...
      <Dialog open={deleteOpen} onOpenChange={(o) => !o && setDeleteOpen(false)}>
        <DialogContent showCloseButton>
          <DialogHeader>
            ...
            <DialogDescription>
              Permanently remove this session and all stored answers. This
              cannot be undone.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter
